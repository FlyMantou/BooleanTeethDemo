...
import com.booleanteeth.demo.App;
import com.booleanteeth.demo.BleAdvertisedData;
import com.booleanteeth.demo.BleUtil;
...
public class BltManager {
    ...
    public interface OnRegisterBltReceiver {
        void onBluetoothDevice(String name,BluetoothDevice device);//搜索到新设备
    ...
    public static String ACTION_FOUND = "ACTION_FOUND";
    public static String ACTION_BOND_STATE_CHANGED = "ACTION_BOND_STATE_CHANGED";
    public static String ACTION_SCAN_MODE_CHANGED = "ACTION_SCAN_MODE_CHANGED";
    public static String ACTION_STATE_CHANGED = "ACTION_STATE_CHANGED";
    /**
     * 注册广播来接收蓝牙配对信息
     *
     * @param context
    ...
    public void registerBltReceiver(Context context, OnRegisterBltReceiver onRegisterBltReceiver) {
        ...
        intent.addAction(ACTION_FOUND);//搜索发现设备
        intent.addAction(ACTION_BOND_STATE_CHANGED);//状态改变
        intent.addAction(ACTION_SCAN_MODE_CHANGED);//行动扫描模式改变了
    ...
    private BroadcastReceiver searchDevices = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_FOUND.equals(action)) {
                ...
                onRegisterBltReceiver.onBluetoothDevice(name,device);
            }
            //状态改变时
    ...
    private boolean startSearthBltDevice(Context context) {
      ...
      //  getmBluetoothAdapter().startDiscovery();
        getmBluetoothAdapter().startLeScan(scanCallback);
        //计时5秒后，停止扫描
        new Thread(){
            @Override
            public void run() {
                try {
                    sleep(5000);
                    getmBluetoothAdapter().stopLeScan(scanCallback);
    ...
    BluetoothAdapter.LeScanCallback scanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
            ...
            Log.i("huanghai","startLeScan发现设备-->"+deviceName);
            Log.i("huanghai","startLeScan发现设备-->"+badata.getUuids());
            if (deviceName==null)
                return;
